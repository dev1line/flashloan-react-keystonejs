...
type Guidetatus = 'idle' | 'coming_soon' | 'live' | 'finished'
...
const getRibbonComponent = (status: Guidetatus, t: any) => {
...
const Header = styled(CardHeader)<{ ifoId: string }>`
  ...
  background-image: ${({ ifoId }) => `url('/images/Guide/${ifoId}-bg.svg')`};
`
...
const IfoFoldableCard: React.FC<IfoFoldableCardProps> = ({ ifo, publicIfoData, walletIfoData, isInitiallyVisible }) => {
  ...
  const Ribbon = getRibbonComponent( "coming_soon", t)
  const isActive = ifo.isActive
