...
import { setReverseCoin } from 'state/predictions'
...
const CardHeader: React.FC<CardHeaderProps> = ({ status, title, epoch, icon, hide, isExchange = false , handleSelect}) => {
  ...
  const reverseToken = useSelector((state: State) => state.predictions.swiperList[0].token)
  const exchangeProvider = useSelector((state: State) => state.predictions.swiperList[epoch].exchange)
  const dataSelect = isExchange ? exchangeProvider : reverseToken;
  const data = useSelector((state: State) => state.predictions.swiperList);
